...
/** 邮件模板示例文件名（文件名含节点标志「上线审核邮件」即被 MailModal 命中） */
export const SEED_MAIL_TEMPLATE_NAME = "01-上线审核邮件.md";

/** 示例需求笔记文件名（需求目录下演示数据） */
export const SEED_SAMPLE_NOTE_NAME = "示例需求.md";

/** 极简规则文件内容（1 个示例环节 = 内置兜底「上线审核」；解析结构与内置默认一致，可自由增删改） */
...
export const SEED_MAIL_TEMPLATE = `# 01 上线审核邮件模板

> 节点邮件模板示例：<模板目录>/邮件模板/ 下文件名包含节点 frontmatter 邮件标志即被插件命中。
> 未找到模板的节点自动使用通用草稿。复制本文件可创建其他节点模板（如 02-其他环节邮件.md）。
